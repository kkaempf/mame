...
#ifndef MAME_INCLUDES_GENIE3S_H
#define MAME_INCLUDES_GENIE3S_H
...
class genie3s_state : public driver_device
{
public:
	genie3s_state(const machine_config &mconfig, device_type type, const char *tag)
	...
	void genie3s(machine_config &config);
...
private:
	...
	uint32_t screen_update_genie3s(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect);

	void genie3s_io(address_map &map);
	void genie3s_mem(address_map &map);
	void genie3s_bank_dk(address_map &map);
